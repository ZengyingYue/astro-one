...
Build a custom astro-one channel in three steps: subclass, package, install.

## How It Works

astro-one discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `astro-one gateway` starts, it scans:

1. Built-in channels in `astro-one/channels/`
2. External packages registered under the `astro-one.channels` entry point group
...
```
astro-one-channel-webhook/
├── astro-one_channel_webhook/
...
```python
# astro-one_channel_webhook/__init__.py
from astro-one_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# astro-one_channel_webhook/channel.py
import asyncio
...
from astro-one.channels.base import BaseChannel
from astro-one.bus.events import OutboundMessage
...
# pyproject.toml
[project]
name = "astro-one-channel-webhook"
version = "0.1.0"
dependencies = ["astro-one", "aiohttp"]

[project.entry-points."astro-one.channels"]
webhook = "astro-one_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
astro-one plugins list      # verify "Webhook" shows as "plugin"
astro-one onboard           # auto-adds default config for detected plugins
```

Edit `~/.astro-one/config.json`:
...
| `default_config()` (classmethod) | Returns default config dict for `astro-one onboard`. Override to declare your fields. |
...
Override `default_config()` so `astro-one onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `astro-one-channel-{name}` | `astro-one-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `astro-one_channel_{name}` | `astro-one_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/astro-one-channel-webhook
cd astro-one-channel-webhook
pip install -e .
astro-one plugins list    # should show "Webhook" as "plugin"
astro-one gateway         # test end-to-end
```

## Verify

```bash
$ astro-one plugins list
