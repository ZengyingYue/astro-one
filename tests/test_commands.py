...
from astro_one.cli.commands import _make_provider, app
from astro_one.config.schema import Config
from astro_one.providers.litellm_provider import LiteLLMProvider
from astro_one.providers.openai_codex_provider import _strip_model_prefix
from astro_one.providers.registry import find_by_model
...
def mock_paths():
    ...
    with patch("astro_one.config.loader.get_config_path") as mock_cp, \
         patch("astro_one.config.loader.save_config") as mock_sc, \
         patch("astro_one.config.loader.load_config") as mock_lc, \
         patch("astro_one.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "已创建配置" in result.stdout
    assert "已创建工作区" in result.stdout
    assert "astro_one 已就绪" in result.stdout
...
def test_onboard_existing_config_refresh(mock_paths):
    ...
    assert result.exit_code == 0
    assert "配置已存在于" in result.stdout
    assert "配置已刷新" in result.stdout
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("astro_one.channels.registry.discover_all", lambda: {})
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("astro_one.providers.custom_provider.AsyncOpenAI") as mock_async_openai:
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("astro_one.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("astro_one.config.paths.get_cron_dir", return_value=cron_dir), \
         patch("astro_one.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("astro_one.cli.commands._make_provider", return_value=object()), \
         patch("astro_one.cli.commands._print_agent_response") as mock_print_response, \
         patch("astro_one.bus.queue.MessageBus"), \
         patch("astro_one.cron.service.CronService"), \
         patch("astro_one.agent.loop.AgentLoop") as mock_agent_loop_cls:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "astro_one.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("astro_one.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("astro_one.config.paths.get_cron_dir", lambda: config_file.parent / "cron")
    monkeypatch.setattr("astro_one.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("astro_one.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("astro_one.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("astro_one.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("astro_one.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("astro_one.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_gateway_uses_workspace_from_config_by_default(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("astro_one.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr(
        "astro_one.cli.commands.sync_workspace_templates",
        lambda path: seen.__setitem__("workspace", path),
    )
    monkeypatch.setattr(
        "astro_one.cli.commands._make_provider",
...
def test_gateway_workspace_option_overrides_config(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("astro_one.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("astro_one.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr(
        "astro_one.cli.commands.sync_workspace_templates",
        lambda path: seen.__setitem__("workspace", path),
...
def test_gateway_uses_config_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("astro_one.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("astro_one.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("astro_one.session.manager.SessionManager", lambda _workspace: object())
    ...
    monkeypatch.setattr("astro_one.cron.service.CronService", _StopCron)
