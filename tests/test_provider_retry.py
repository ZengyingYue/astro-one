...
from astro_one.providers.base import GenerationSettings, LLMProvider, LLMResponse
...
async def test_chat_with_retry_retries_transient_error_then_succeeds(monkeypatch) -> None:
    ...
    monkeypatch.setattr("astro_one.providers.base.asyncio.sleep", _fake_sleep)
...
@pytest.mark.asyncio
async def test_tools_unsupported_error_retries_without_tools() -> None:
    provider = ScriptedProvider([
        LLMResponse(
            content='Ollama_chatException - {"error":"model does not support tools"}',
            finish_reason="error",
        ),
        LLMResponse(content="ok, no tools"),
    ])
    ...
    assert response.content == "ok, no tools"
    assert provider.calls == 2
    assert provider.last_kwargs["tools"] is None
    assert provider.last_kwargs["tool_choice"] is None


@pytest.mark.asyncio
async def test_stream_tools_unsupported_error_retries_without_tools() -> None:
    provider = ScriptedStreamProvider([
        LLMResponse(content="registry.ollama.ai/library/qwopus does not support tools", finish_reason="error"),
        LLMResponse(content="ok, streamed without tools"),
    ])
    tools = [{"type": "function", "function": {"name": "x", "parameters": {}}}]
    ...
    assert response.content == "ok, streamed without tools"
    assert provider.calls == 2
    assert provider.last_kwargs["tools"] is None
    assert provider.last_kwargs["tool_choice"] is None
    assert provider.last_kwargs["on_tool_call_delta"] is None
