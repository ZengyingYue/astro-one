...
import astro_one.channels.dingtalk as dingtalk_module
from astro_one.channels.dingtalk import DingTalkChannel, NanobotDingTalkHandler
from astro_one.channels.dingtalk import DingTalkConfig
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/astro-one_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/astro-one_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "astro_one.config.paths.get_media_dir",
