...
import astro_one.channels.matrix as matrix_module
from astro_one.bus.events import OutboundMessage
from astro_one.bus.queue import MessageBus
from astro_one.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from astro_one.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("astro_one.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "astro_one.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("astro_one.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "astro_one.channels.matrix.asyncio.create_task", _fake_create_task
    )
