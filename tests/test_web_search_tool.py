...
from astro_one.agent.tools.web import WebSearchTool
...
async def test_brave_search(monkeypatch):
    ...
    result = await tool.execute(query="astro-one", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("astro_one.agent.tools.web.DDGS", MockDDGS, raising=False)
    import astro_one.agent.tools.web as web_mod
