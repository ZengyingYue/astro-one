...
from astro_one.config.loader import load_config, resolve_config_env_vars, save_config
from astro_one.config.paths import get_runtime_subdir
from astro_one.config.schema import MCPServerConfig
...
def _custom_manifest(name: str, cfg: MCPServerConfig) -> dict[str, Any]:
    ...
    return app_manifest(
        app_id=name,
        display_name=name,
        description="Custom MCP server from astro_one config.",
        category="custom",
        source="mcp-custom",
        brand_color="#64748B",
        capabilities=[
