---
name: update-setup
description: One-time setup wizard for the astro_one upgrade skill. Triggers: setup update, configure update, 切设置更新, 初始化更新.
---
...
Use `exec` to run `astroone --version`. Tell the user the current version.

Then collect install clues with `exec`. These commands are best-effort; if one fails,
keep going and show the useful output:

```
command -v astroone || true
python -m pip show astro-one-ai || true
pipx list | sed -n '/astro-one-ai/,+3p' || true
uv tool list | sed -n '/astro-one-ai/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how astro_one was
installed.
...
`question: "Where is your astro_one source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "astro-one-ai[EXTRAS]" --force` |
| pipx | `pipx install --force "astro-one-ai[EXTRAS]"` |
| pip | `python -m pip install --upgrade "astro-one-ai[EXTRAS]"` |
...
---
name: update
description: "Upgrade astro_one to the latest version. Triggers: upgrade astro_one, update astro_one, 升级astro_one, 更新astro_one."
---
...
4. Use `exec` to verify: `astroone --version`
5. Tell the user the new version. Say: "Run `/restart` to restart astro_one and apply the update. If `/restart` is unavailable in this channel, restart the astro_one process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade astro_one' when you want to update."
