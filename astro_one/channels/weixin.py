...
from astro_one.config.paths import get_media_dir, get_runtime_subdir
from astro_one.config.schema import Base
from astro_one.utils.helpers import split_message
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.astro_one/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                self.logger.error("login failed. Run 'astro_one channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"astro_one-{uuid.uuid4().hex[:12]}"
