...
def discover_channel_names() -> list[str]:
    ...
    import astro_one.channels as pkg
...
def load_channel_class(module_name: str) -> type[BaseChannel]:
    ...
    from astro_one.channels.base import BaseChannel as _Base

    mod = importlib.import_module(f"astro_one.channels.{module_name}")
    for attr in dir(mod):
        obj = getattr(mod, attr)
        if isinstance(obj, type) and issubclass(obj, _Base) and obj is not _Base:
            return obj
    raise ImportError(f"No BaseChannel subclass in astro_one.channels.{module_name}")


def discover_plugins(enabled_names: set[str] | None = None) -> dict[str, type[BaseChannel]]:
    """Discover external channel plugins registered via entry_points.

    If *enabled_names* is provided, only plugins whose name is in that set are loaded.
    """
    from importlib.metadata import entry_points

    plugins: dict[str, type[BaseChannel]] = {}
    for ep in entry_points(group="astro_one.channels"):
