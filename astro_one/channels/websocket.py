"""WebSocket server channel: astro_one acts as a WebSocket server and serves connected clients."""
...
from astro_one.agent.tools.mcp import request_mcp_reload
from astro_one.bus.events import OUTBOUND_META_AGENT_UI, OutboundMessage
from astro_one.bus.queue import MessageBus
from astro_one.channels.base import BaseChannel
from astro_one.command.builtin import builtin_command_palette
from astro_one.config.paths import get_media_dir, get_workspace_path
from astro_one.config.schema import Base
from astro_one.session.goal_state import goal_state_ws_blob
from astro_one.session.webui_turns import websocket_turn_wall_started_at
from astro_one.utils.helpers import safe_filename
from astro_one.utils.media_decode import (
    FileSizeExceeded,
    save_base64_data_url,
)
from astro_one.utils.subagent_channel_display import scrub_subagent_messages_for_channel
from astro_one.webui.settings_api import (
    WebUISettingsError,
...
from astro_one.webui.cli_apps_api import (
    cli_apps_action,
    cli_apps_payload,
    normalize_cli_app_mentions,
)
from astro_one.webui.mcp_presets_api import (
    mcp_presets_settings_action,
    normalize_mcp_preset_mentions,
)
from astro_one.webui.sidebar_state import (
    read_webui_sidebar_state,
    write_webui_sidebar_state,
)
from astro_one.webui.thread_disk import delete_webui_thread
from astro_one.webui.transcript import (
...
_MCP_VALUES_HEADER = "X-Astro_one-MCP-Values"
...
class WebSocketConfig(Base):
    ...
    - ``token_issue_path``: If non-empty, **GET** (HTTP/1.1) to this path returns JSON
      ...
      astro_one and shares the asyncio loop, use a thread or async HTTP client for GET—do not call
      blocking ``urllib`` or synchronous ``httpx`` from inside a coroutine.
    - ``token_issue_secret``: If non-empty, token requests must send ``Authorization: Bearer <secret>`` or
      ``X-Astro_one-Auth: <secret>``.
    ...
    # client-side Worker normalization (see webui Composer). 4 x 6 MB x 1.37
...
def _issue_route_secret_matches(headers: Any, configured_secret: str) -> bool:
    ...
    header_token = headers.get("X-Astro_one-Auth") or headers.get("x-astro_one-auth")
    if not header_token:
        return False
...
class WebSocketChannel(BaseChannel):
    ...
    async def start(self) -> None:
        from astro_one.utils.logging_bridge import redirect_lib_logging
