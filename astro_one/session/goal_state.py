...
def runner_wall_llm_timeout_s(
    ...
    *,
    ...
    """Wall-clock cap for :class:`~astro_one.agent.runner.AgentRunner` when streaming an LLM.

    Returns ``0.0`` to disable ``asyncio.wait_for`` around the request when a sustained goal is
    active; ``None`` means use ``ASTRO_ONE_LLM_TIMEOUT_S``. Pass in-memory ``metadata`` when the
    caller already holds :attr:`~astro_one.session.manager.Session.metadata` for this turn.
    """
