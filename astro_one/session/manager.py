...
from astro_one.config.paths import get_legacy_sessions_dir
from astro_one.utils.helpers import ensure_dir, safe_filename
...
class Session:
    ...
    def _annotate_message_time(message: dict[str, Any], content: Any) -> Any:
        ...
        Only annotate user turns to avoid training the model to reproduce prefixes.
        """
    ...
    def get_history(
        ...
        *,
        ...
        for message in sliced:
            ...
            # Synthesize image breadcrumb from persisted media kwarg.
            media = message.get("media")
            if role == "user" and isinstance(media, list) and media and isinstance(content, str):
                breadcrumbs = "\n".join(
                    _image_placeholder_text(p) for p in media if isinstance(p, str) and p
                )
        ...
        if max_tokens > 0 and out:
            from astro_one.utils.helpers import estimate_message_tokens
...
class SessionManager:
    ...
    def _get_legacy_session_path(self, key: str) -> Path:
        """Legacy global session path (~/.astro_one/sessions/)."""
    ...
    def save(self, session: Session, *, fsync: bool = False) -> None:
        ...
        default but should be enabled during graceful shutdown.
        """
    ...
    def flush_all(self) -> int:
        ...
        sessions are logged but do not prevent other sessions from being flushed.
        """
        flushed = 0
    ...
    def list_sessions(self) -> list[dict[str, Any]]:
        ...
        Returns:
            List of session info dicts with key, title, preview, and timestamps.
        """
        sessions = []
            ...
            try:
                ...
                if repaired is not None:
                    sessions.append({
                        ...
                        "preview": next(
                            (
                                _text_preview(msg.get("content"))
                                for msg in repaired.messages
                                if _text_preview(msg.get("content"))
                            ),
                            "",
                        ),
                        "path": str(path)
