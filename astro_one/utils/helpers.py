"""Utility functions for astro_one."""
...
def strip_think(text: str) -> str:
    """Remove thinking blocks and unclosed trailing tags from model output."""
    ...
    text = re.sub(r"<\|(?:endoftext|im_start|im_end)\|>[\s\S]*$", "", text)
    partial_control_tag = (
        r"</?(?:t|th|thi|thin|think|tho|thou|thoug|though|thought)>?"
        r"|<\|?(?:c|ch|cha|chan|chann|channe|channel)(?:\|?>?)?"
        r"|<\|?(?:e|en|end|endo|endof|endoft|endofte|endoftex|endoftext)(?:\|?>?)?"
    )
...
_TOOL_RESULTS_DIR = ".astro-one/tool-results"
...
def build_status_content(
    *,
    ...
    lines = [
        f"\U0001f680 astro-one v{version}",
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    try:
        tpl = pkg_files("astro_one") / "templates"
    except Exception:
        return []
    if not tpl.is_dir():
        return []
