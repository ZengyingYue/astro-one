...
class ProviderSpec:
    ...
    display_name: str = ""  # shown in `astro_one status`
    ...
    # Optional provider-specific thinking/reasoning request body styles.
    thinking_style: str = ""
    ...
    def backend(self) -> str:
        """Provider implementation used by the factory.

        Older factory code expects a ``backend`` attribute. Keep that as a
        derived property so the registry remains the single source of truth.
        """
        if self.name in {
            "custom",
            "azure_openai",
