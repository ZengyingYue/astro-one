"""Interactive onboarding questionnaire for astro-one."""
...
from astro_one.cli.models import (
    format_token_count,
    get_model_context_limit,
    get_model_suggestions,
)
from astro_one.config.loader import get_config_path, load_config
from astro_one.config.schema import Config, ModelPresetConfig
...
def _show_main_menu_header() -> None:
    ...
    console.print(
        Align.center(f"{__logo__} [bold cyan]astro-one[{__version__}][/bold cyan]")
    )
    console.print()
...
def _handle_fallback_models_field(
    ...
    from astro_one.config.schema import InlineFallbackConfig
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from astro_one.config.schema to get
    ...
    from astro_one.config.schema import AgentDefaults
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"astro_one.channels.{name}")
